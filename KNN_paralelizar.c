...
/***** PARALELIZAÇÃO - CRIAÇÃO E PREENCHIMENTO DAS MATRIZES ************************************************************/
...
/************** Possível paralelização no cálculo das distâncias ********************************************************/
    for (int i = 0; i < n; i++) {
        double soma = 0.0;
        for (int j = 0; j < largura; j++) {
...
void encontrar_k_menores(double *distancias, int n, int k, int *indices) {
    for (int i = 0; i < k; i++) {
        ...
        /*****************possivel paralelização, mas é dificil pela questão da comparação de achar menor valor*********/
        for (int j = 0; j < n; j++) {
...
/************** Possível paralelização no cálculo do erro absoluto médio *************************************************/
    for (int i = 0; i < tamanho; i++) {
        soma += fabs(y_real[i] - y_previsto[i]);
    }
/***********************************************************************************************************************/
// aqui teria que tornar soma uma variavel global para todas as threads igual está no slide 63
